...
async function fetchWUForecast(lat:number,lon:number,unit:"F"|"C"):Promise<{all:HourlyPoint[];day:ForecastDay|null}>{if(!WU_KEY)return fetchOMForecast(lat,lon,'UTC');try{const units=unit==='F'?'e':'m';const json=await fetch(`https://api.weather.com/v3/wx/forecast/hourly/1day?geocode=${lat},${lon}&units=${units}&language=en-US&format=json&apiKey=${WU_KEY}`,{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('WU');return r.json();});const times:string[]=json.validTimeLocal??[];const temps:number[]=json.temperature??[];if(!times.length)return fetchOMForecast(lat,lon,'UTC');const toC=(t:number)=>unit==='F'?(t-32)*5/9:t;const all=times.map((t,i)=>({time:t.replace(/[+-]\d{4}$|Z$/,''),tempC:toC(temps[i])})).filter(p=>validTemp(p.tempC));const allC=all.map(p=>p.tempC);return{all,day:allC.length?{maxC:Math.max(...allC),minC:Math.min(...allC),maxDisplay:0,minDisplay:0}:null};}catch{return fetchOMForecast(lat,lon,'UTC');}}
async function fetchOMForecast(lat:number,lon:number,timezone:string):Promise<{all:HourlyPoint[];day:ForecastDay|null}>{try{const json=await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&hourly=temperature_2m&daily=temperature_2m_max,temperature_2m_min&timezone=${encodeURIComponent(timezone)}&forecast_days=2&models=ecmwf_ifs025`,{cache:'no-store'}).then(r=>r.json());const todayStr=new Date().toLocaleDateString('en-CA',{timeZone:timezone});const all=(json.hourly.time as string[]).map((t,i)=>({time:t,tempC:json.hourly.temperature_2m[i] as number})).filter(p=>p.time.startsWith(todayStr)&&validTemp(p.tempC));const maxC=json.daily?.temperature_2m_max?.[0]??(all.length?Math.max(...all.map(p=>p.tempC)):0);const minC=json.daily?.temperature_2m_min?.[0]??(all.length?Math.min(...all.map(p=>p.tempC)):0);return{all,day:{maxC,minC,maxDisplay:0,minDisplay:0}};}catch{return{all:[],day:null};}}
