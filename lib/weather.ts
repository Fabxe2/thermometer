...
// FIX #4: usa toLocaleTimeString (no toLocaleString que devuelve fecha completa)
...
// FIX #1: filtra por fecha local de la CIUDAD, no del servidor (UTC en Railway)
async function fetchMetarHistory(station: string, timezone: string): Promise<HourlyPoint[]> {
  try {
    ...
    const now = new Date();
    // todayStr calculado en la timezone de la CIUDAD (no del servidor UTC)
    const todayStr = now.toLocaleDateString('en-CA', { timeZone: timezone });

    return data
      .filter(m => m.temp != null)
      .map(m => {
        const obsUTC = new Date(String(m.reportTime ?? m.obsTime ?? ''));
        // Fecha local en timezone de la ciudad
        const localDateStr = obsUTC.toLocaleDateString('en-CA', { timeZone: timezone });
        // Hora local en la ciudad (toLocaleTimeString, no toLocaleString)
        const localTimeStr = obsUTC.toLocaleTimeString('en-US', {
        ...
        return {
          localDateStr,
          time: localDateStr + 'T' + String(h).padStart(2,'0') + ':' + String(min).padStart(2,'0') + ':00',
          tempC: Number(m.temp),
          obsUTC,
        };
      })
      .filter(p => p.localDateStr === todayStr && p.obsUTC <= now)
      .sort((a, b) => a.obsUTC.getTime() - b.obsUTC.getTime())
      .map(({ time, tempC }) => ({ time, tempC }));
  } catch { return []; }
}
...
// FIX #3 (parcial): forecast_days=2 sin past_days -> daily[0]=hoy siempre
async function fetchOMFallback(city: City): Promise<{ all: HourlyPoint[]; day: ForecastDay | null }> {
  try {
    const json = await fetch(
      ...
      `&timezone=${encodeURIComponent(city.timezone)}&forecast_days=2`,
      { cache: 'no-store' }
    ).then(r => r.json());
    ...
    // forecast_days=2 sin past_days -> daily[0]=hoy
    const maxC = json.daily?.temperature_2m_max?.[0] ?? (all.length ? Math.max(...all.map(p=>p.tempC)) : 0);
    const minC = json.daily?.temperature_2m_min?.[0] ?? (all.length ? Math.min(...all.map(p=>p.tempC)) : 0);
    return { all, day: { maxC, minC, maxDisplay: 0, minDisplay: 0 } };
...
export async function fetchWeatherData(city: City): Promise<WeatherData> {
  ...
  const forecastAll: HourlyPoint[] = forecastResult.all ?? [];
  ...
  if (metarObs) {
    return { current: applyUnit(metarObs, city), obsHourly, forecastHourly: forecastAll, forecast };
  }
  const omC = await fetchOMCurrent(city);
  return { current: omC ? applyUnit(omC, city) : null, obsHourly, forecastHourly: forecastAll, forecast };
}