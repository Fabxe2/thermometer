...
// Current METAR from tgftp (no-cache, always fresh)
...
// US cities: NWS observations API — single fast request for full day history
...
// International cities: tgftp cycle files — one file per UTC hour, 24 parallel fetches
// Each file has ALL stations worldwide. 3s timeout per file to avoid blocking.
async function fetchTgftpObsHistory(station: string): Promise<HourlyPoint[]> {
  const now = new Date();
  const hours = Array.from({ length: 24 }, (_, i) => (now.getUTCHours() - i + 24) % 24);

  const fetchOne = async (h: number): Promise<HourlyPoint | null> => {
    const ctrl = new AbortController();
    const t = setTimeout(() => ctrl.abort(), 3000);
    try {
      const hh = String(h).padStart(2, '0');
      const res = await fetch(
        `https://tgftp.nws.noaa.gov/data/observations/metar/cycles/${hh}Z.TXT`,
        { signal: ctrl.signal, next: { revalidate: 3600 } as RequestInit['next'] }
      );
      clearTimeout(t);
      if (!res.ok) return null;
      const text = await res.text();
      const re = new RegExp(`^(${station}\\s+\\d{6}Z[^\\n]*)$`, 'm');
      const match = text.match(re);
      if (!match) return null;
      const raw = match[1].trim();
      const tempC = parseTempFromMetar(raw); if (tempC == null) return null;
      const tm = raw.match(/\b(\d{2})(\d{2})(\d{2})Z\b/); if (!tm) return null;
      const obs = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(),
        parseInt(tm[1],10), parseInt(tm[2],10), parseInt(tm[3],10)));
      return { time: obs.toISOString(), tempC };
    } catch { clearTimeout(t); return null; }
  };

  const results = await Promise.all(hours.map(fetchOne));
  const seen = new Set<number>();
  return results
    .filter((r): r is HourlyPoint => r !== null)
    .filter(r => { const h = new Date(r.time).getUTCHours(); if (seen.has(h)) return false; seen.add(h); return true; })
...
// Open-Meteo forecast (international + fallback)
async function fetchOMForecast(city: City): Promise<{ hourly: HourlyPoint[]; day: ForecastDay | null }> {
  try {
    ...
    return {
      hourly: (json.hourly.time as string[]).map((t: string, i: number) => ({ time: t, tempC: json.hourly.temperature_2m[i] })),
      day: { maxC: json.daily.temperature_2m_max[0], minC: json.daily.temperature_2m_min[0], maxDisplay: 0, minDisplay: 0 },
    };
...
async function fetchOMCurrent(city: City): Promise<WeatherObs | null> {
  try {
    ...
    const tempC: number = json.current.temperature_2m; const now = new Date();
...
export async function fetchWeatherData(city: City): Promise<WeatherData> {
  if (city.region === 'us') {
    // US: 1 request for obs history (NWS), parallel with forecast + current
  ...
  // International: tgftp current + 24 cycle files (3s timeout each) + OM forecast
  const [metarObs, obsHourly, omResult] = await Promise.all([
    fetchTgftpMetar(city.station, city.timezone),
    fetchTgftpObsHistory(city.station),
    fetchOMForecast(city),
  ]);
  const forecast = omResult.day ? applyForecastUnit(omResult.day, city) : null;
  if (metarObs) return { current: applyUnit(metarObs, city), obsHourly, forecastHourly: omResult.hourly, forecast };
  const omC = await fetchOMCurrent(city);
  return { current: omC ? applyUnit(omC, city) : null, obsHourly, forecastHourly: omResult.hourly, forecast };
}