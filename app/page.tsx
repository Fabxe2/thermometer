...
async function CityCard({ city }: { city: (typeof CITIES)[0] }) {
  ...
  const polyData = await fetchPolymarketData(city, current?.tempDisplay ?? 0);
      ...
      <div style={{ padding:'16px 0', borderBottom:'1px solid var(--color-rule)' }}>
        ...
        {topBucket && (
          <div style={{ marginTop:6, display:'flex', alignItems:'center', gap:8 }}>
            <span style={{ fontSize:12, fontFamily:'monospace', color:'var(--color-accent)' }}>{labelWithUnit(topBucket.label)}</span>
