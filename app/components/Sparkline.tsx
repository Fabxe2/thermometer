...
export type ChartPoint = { hour: number; observed?: number; forecast?: number; };
...
function CustomTooltip({ active, payload, label, unit }: {
  ...
  return (
    <div style={{ background:"rgba(10,10,10,0.95)", border:"1px solid rgba(255,255,255,0.12)",
      borderRadius:6, padding:"8px 12px", fontSize:11, fontFamily:"monospace", lineHeight:1.9, pointerEvents:"none" }}>
      <div style={{color:"rgba(255,255,255,0.4)", marginBottom:3}}>{hh}:00 {ampm}</div>
      {obs && (
        <div style={{color:"#ffffff", display:"flex", justifyContent:"space-between", gap:20}}>
          <span>OBS</span><strong>{obs.value}{sym}</strong>
        </div>
      )}
      {fcast && (
        <div style={{color:"rgba(255,255,255,0.45)", display:"flex", justifyContent:"space-between", gap:20}}>
          <span>FCST</span><span>{fcast.value}{sym}</span>
        </div>
      )}
    </div>
  );
...
export default function Sparkline({ data, height = 300, unit = "C", cityHour }: Props) {
  ...
  // FIX #2: usar hora de la CIUDAD pasada como prop, no hora local del browser
  const nowH = cityHour ?? new Date().getHours();
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <ComposedChart data={data} margin={{top:8, right:12, left:0, bottom:0}}>
        ...
        <YAxis domain={domain} ticks={yTicks} tickFormatter={v => v + sym}
        ...
        <ReferenceLine x={nowH} stroke="rgba(255,255,255,0.2)" strokeWidth={1} strokeDasharray="3 3"
