...
{es.slice(0,12).map((e,i)=>{const v=toD(e.tempC,unit);const pct=Math.round(((v-mn)/rng)*100);
const last=i===Math.min(es.length,12)-1;
return(<div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"6px 0",borderBottom:last?"none":"0.5px solid rgba(255,255,255,0.06)"}}><span style={{fontSize:11,fontFamily:"monospace",color:"rgba(255,255,255,0.35)",minWidth:60,flexShrink:0}}>{e.time}</span>
...
<div style={{height:"100%",width:pct+"%",minWidth:4,borderRadius:2,background:e.live?"#fff":"rgba(255,255,255,0.45)"}}/></div>
{e.live?<span style={{fontSize:10,color:"#4ade80",fontFamily:"monospace",flexShrink:0}}>● live</span>
